...
function localNetworkDeployments(deployer) {
  ...
  let addresses = []
  return deployer.deploy(StandardERC20, 1000, "Fake USDC", 18, "fUSDC").then(deployOne => {
...
async function getERC20s(deployer, network) {
  ...
  if (localNetworks.includes(network)) {
    console.log(`Local build from network: ${network}, deploying fake ERC20 tokens!`)
    ...
    console.log(`Real network detected: ${network}, using real ERC20 addresses!`)
